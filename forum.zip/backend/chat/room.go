...
func (r *Room) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	fmt.Println("w :", w, " r:", r, "/////////////////", "upgrade: ", upgrader)
...
func (r *Room) Run() {
	...
	for {
		...
		case msg := <-r.forward:
			...
			_, chatError := db.Exec(`INSERT INTO messages(sender, receiver, message, time, status) values(?,?,?, datetime('now','localtime'), ?)`, data.Sender, data.Receiver, data.Message, data.Status)
			if chatError != nil {
...
// and go throught ime setting the value to true is just a handy, low- memory way of storing the reference.
